...
def update_filter_controls(stored_df, stored_column_types, selected_column):
    ...
    Returns:
        Tuple of (numeric_group_visible, categorical_group_visible, date_group_visible, 
                 numeric_min_value, numeric_max_value, categorical_choices)
    """
    try:
        if not selected_column or stored_df is None or not stored_column_types:
            return (gr.update(visible=False), gr.update(visible=False), gr.update(visible=False),
                   0, 100, gr.update(choices=[]))
        ...
        if col_type == 'numeric':
            ...
            return (gr.update(visible=True), gr.update(visible=False), gr.update(visible=False),
                   min_val, max_val, gr.update(choices=[]))
            ...
            return (gr.update(visible=False), gr.update(visible=True), gr.update(visible=False),
                   0, 100, gr.update(choices=unique_vals, value=[]))
...
def create_dashboard():
    ...
    with gr.Blocks(title="Business Intelligence Dashboard") as demo:
        ...
        with gr.Tab("🔍 Filter & Explore"):
            ...
            gr.Markdown("Select a column and apply filters based on its type")
            
            with gr.Row():
                with gr.Column(scale=1):
                    setup_filters_btn = gr.Button("Setup Filters", variant="primary")
                    ...
                    with gr.Group(visible=False) as numeric_filter_group:
                        gr.Markdown("**Numeric Range Filter**")
                        numeric_min = gr.Number(label="Minimum Value", value=0)
                        numeric_max = gr.Number(label="Maximum Value", value=100)
                    ...
                    with gr.Group(visible=False) as categorical_filter_group:
                        ...
                        categorical_values = gr.Dropdown(
                            label="Select Values to Include",
                            choices=[],
                            multiselect=True,
                            interactive=True
                        )
                    ...
                    with gr.Group(visible=False) as date_filter_group:
                        ...
                        date_start = gr.Textbox(label="Start Date (YYYY-MM-DD)", value="")
                        date_end = gr.Textbox(label="End Date (YYYY-MM-DD)", value="")
                    
                    apply_filter_btn = gr.Button("Apply Filter", variant="secondary")
                    reset_filter_btn = gr.Button("Reset All Filters")
                
                with gr.Column(scale=2):
                    filtered_count = gr.Textbox(label="Filtered Results", interactive=False)
            ...
            filter_column.change(
                ...
                outputs=[numeric_filter_group, categorical_filter_group, date_filter_group,
                        numeric_min, numeric_max, categorical_values]
            )
            ...
            apply_filter_btn.click(
                ...
                inputs=[stored_df, stored_column_types, filter_column,
                       numeric_min, numeric_max, categorical_values, date_start, date_end],
                outputs=[filtered_count, filtered_preview]
            )
            
            reset_filter_btn.click(
                fn=reset_filters,
                inputs=[stored_df],
                outputs=[filtered_count, filtered_preview, filter_column]
            )
